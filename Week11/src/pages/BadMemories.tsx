...
const BadMemories: React.FC = () => {
    ...
    const badMemories = memories.filter(memory => memory.type == 'bad');
            ...
            <IonContent>
                <IonGrid>
                    ...
                    {badMemories.map(memory => (
                        <MemoryItem key={memory.id} id={memory.id} base64Url={memory.imageUrl} title={memory.title} lat={memory.lat} long={memory.lng}/>
                    ))}
                </IonGrid>
            </IonContent>
        </IonPage>
